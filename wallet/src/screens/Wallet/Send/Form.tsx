...
import SatsPathVerified from '../../../components/SatsPathVerified'
import { SatsPathContext } from '../../../providers/satspath'
import { isSatsPathAlias, extractRailFromQuote, getRailIcon, getRailLabel, formatFee, buildQrForRail } from '../../../lib/satspath'
import type { SatsPathQuoteResponse } from '../../../providers/satspath'
...
export default function SendForm() {
  ...
  const { initialized: satspathInitialized, resolveAndQuote } = useContext(SatsPathContext)
  const {
    assetBalances,
  ...
  // SatsPath state
  const [satspathQuote, setSatspathQuote] = useState<SatsPathQuoteResponse | null>(null)
  const [satspathLoading, setSatspathLoading] = useState(false)
  const [satspathError, setSatspathError] = useState('')
  ...
  const smartSetError = (str: string) => {
    // Don't show ASP unreachable error when using SatsPath aliases
    if (str === '' && isSatsPathAlias(recipient)) {
      setError('')
      return
    }
  ...
  // SatsPath alias resolution and quote
  useEffect(() => {
    if (!satspathInitialized) return
    if (!recipient || !isSatsPathAlias(recipient)) {
    ...
    const amountSats = sendInfo.satoshis ?? 0
    if (amountSats <= 0) {
      setSatspathQuote(null)
      setSatspathError('')
      return
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const result = await resolveAndQuote(recipient, amountSats)
        if (cancelled) return

        if (result.status === 'ok' && result.quote) {
          setSatspathQuote(result.quote)
          setSatspathError('')
        } else {
          setSatspathQuote(null)
          setSatspathError(result.error || 'Unable to resolve alias')
        }
      } catch (err) {
        if (!cancelled) {
          consoleError(err, 'SatsPath resolution failed')
          setSatspathQuote(null)
          setSatspathError(extractError(err))
        }
      } finally {
        if (!cancelled) setSatspathLoading(false)
      }
    }, 600)
  ...
  }, [recipient, sendInfo.satoshis, satspathInitialized, resolveAndQuote])

  // Sync sendInfo when SatsPath quote changes (initial load or rail switch)
  useEffect(() => {
    if (!satspathQuote || satspathQuote.status !== 'ok') return
    const rail = extractRailFromQuote(satspathQuote)
    setSendInfo((prev) => ({
      ...prev,
      address: rail.address,
      arkAddress: rail.arkAddress,
      invoice: undefined,
      lnUrl: rail.lnUrl,
  ...
  useEffect(() => {
    if (isSatsPathAlias(recipient)) return // don't show ASP errors for SatsPath aliases
  ...
  }, [aspInfo.unreachable, aspInfo.outdated, recipient])
  ...
  // Handle selecting a rail from SatsPath quote
  const handleSelectRail = (railType: string) => {
    setSatspathQuote((prev) => {
      if (!prev) return prev
      const method = prev.available_methods?.find((m) => m.type === railType)
      if (!method) return prev
      const amountSats = sendInfo.satoshis ?? 0
      const qr = buildQrForRail(method, amountSats)
      // Update sendInfo with the payment data for this rail
      const rail = extractRailFromQuote({ ...prev, selected_method: { type: railType as any }, qr })
      setSendInfo((s) => ({
        ...s,
        address: rail.address,
        arkAddress: rail.arkAddress,
      ...
      return { ...prev, selected_method: { type: railType as any }, qr }
    })
  }
          ...
          <Padded>
            <FlexCol gap='1.25rem' className='send-form-stack'>
              ...
              {brantaPayment
                ...
                : null}
              {/* SatsPath Quote Display */}
              {satspathLoading ? (
                <Shadow>
                  <FlexRow between padding='0.75rem'>
                    <FlexCol gap='0.1rem'>
                      <Text smaller>Resolving alias...</Text>
                      <Text smaller color='neutral-500'>
                        Looking up payment route
                      </Text>
                    </FlexCol>
                    <LoadingLogo text='' />
                  </FlexRow>
                </Shadow>
              ) : satspathQuote && satspathQuote.status === 'ok' ? (
                <Shadow>
                  <FlexCol gap='0.75rem' padding='0.75rem'>
                    <FlexRow between>
                      <FlexCol gap='0.25rem'>
                        <FlexRow gap='0.5rem'>
                          <Text smaller bold>SatsPath Route</Text>
                          <SatsPathVerified showText={false} size='small' />
                        </FlexRow>
                        <Text smaller color='neutral-500'>
                          {satspathQuote.recipient?.alias ?? recipient}
                        </Text>
                      </FlexCol>
                      <Text smaller color='neutral-500'>
                        {formatFee(satspathQuote.fee_sats)}
                      </Text>
                    </FlexRow>
                    <FlexRow gap='0.5rem'>
                      {(['Lightning', 'Ark', 'Onchain'] as const)
                        .filter((rail) => satspathQuote.available_methods?.some((m) => m.type === rail))
                        .map((rail) => {
                        const isSelected = satspathQuote.selected_method?.type === rail
                        return (
                          <button
                            key={rail}
                            type='button'
                            ...
                            style={{
                              ...
                              padding: '0.5rem',
                              borderRadius: '0.5rem',
                              border: isSelected ? '2px solid var(--satspath-red)' : '1px solid var(--neutral-300)',
                              background: isSelected ? 'var(--satspath-red-10)' : 'transparent',
                              cursor: 'pointer',
                              display: 'flex',
                          ...
                          >
                            <span style={{ fontSize: '1.25rem' }}>{getRailIcon(rail)}</span>
                            <Text small color={isSelected ? 'white' : 'neutral-500'}>
                              {getRailLabel(rail)}
                            </Text>
                          </button>
                        )
                ...
                <Text color='red' smaller>{satspathError}</Text>
              ) : null}
